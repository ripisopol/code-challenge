...
const Hero: React.FC<HeroProps> = ({ title, description, button, imageUrl, imageWidth, imageHeight }) => {
    return (
        <section className="min-h-full text-white bg-pink-500 body-font  pt-12">
            <div className="container mx-auto px-5 pt-12 md:flex md:flex-row md:items-center">
                <div className="md:w-1/2 md:pr-4">
                    <div className="flex flex-col md:items-start md:text-left mb-16 md:mb-0 items-center text-center">
                        <h1 className="font-bold mb-8 text-white text-6xl md:text-7xl ">{title}</h1>
                        <p className="mb-8 leading-relaxed">{description}</p>
                        {button && (
                            <Link href="/about" className="mb-20 flex justify-center rounded-full border border-white bg-pink-500 text-white px-8 py-2 hover:underline">
                                {button}
                            </Link>
                        )}
                    </div>
